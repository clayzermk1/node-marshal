...
Marshal = (function () {
  ...
  Marshal.prototype._parseFloat = function () {
    var floatLength = this.buffer.readInt8(this._index++);
    if (floatLength === 0) {
      floatLength = 0;
    }
    else if (floatLength >= 6) {
      floatLength = floatLength - 5;
    }
    else if (floatLength <= -6) {
      floatLength = floatLength + 5;
    }
    var tempBuf = this.buffer.slice(this._index, this._index + floatLength);
    this._index = this._index + floatLength;
    if (tempBuf.toString('utf8') === 'inf') {
      return Infinity;
    }
    if (tempBuf.toString('utf8') === '-inf') {
      return -Infinity;
